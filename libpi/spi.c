...
static void start_transfer(unsigned nbytes) {
	mem_barrier();		// paranoid, but would suck to debug if needed.
	 ...
	 */
	mem_barrier();		// if we ran cached i think we'd have to 
...
void spi_init(unsigned chip_select, unsigned clk_div) {
	...
	mem_barrier();
		...
		if(chip_select == SPI_CE0) {
			gpio_set_function(GPIO_PIN8, GPIO_FUNC_ALT0); 
			chip = 0b00;
		// SPI0_CE1_N
		} else if(chip_select == SPI_CE1) {
			gpio_set_function(GPIO_PIN7, GPIO_FUNC_ALT0); 
			chip = 0xb01;
		} else
